"""ReAct-style baseline for HLE (Humanity's Last Exam) gold subset, no tools, multithreaded, resumable."""
...
SYSTEM_PROMPT = (
    ...
    "Reason step by step (Observe, Think, Act) in plain text, but do not use any tools. "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ReAct baseline for HLE (no tools, multithreaded, resumable)")
    ...
    output_path = args.output or os.path.join("results", model_name, "react", "hle_result.json")
    ...
    print(f"ReAct: {correct}/{len(results)} correct -> {output_path}")
