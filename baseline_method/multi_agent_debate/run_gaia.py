...
def main() -> None:
    parser = argparse.ArgumentParser(description="MAD baseline for GAIA")
    ...
    output_path = args.output or os.path.join("results", model_name, "multi_agent_debate", "gaia_result.json")
    ...
    pbar = tqdm(dataset, desc="MAD")
    for example in pbar:
        question = example["question"]
        gold = example.get("gold_answer")
        result = debate_solve(question)
