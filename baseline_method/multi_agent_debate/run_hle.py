"""Multi-agent debate baseline for HLE (Humanity's Last Exam) gold subset, no tools, multithreaded, resumable."""
...
from baseline_method.multi_agent_debate.agents import debate_solve_text
...
def process_example(example: dict) -> tuple[dict, bool]:
    ...
    result = debate_solve_text(question)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MAD baseline for HLE (no tools, multithreaded, resumable)")
    ...
    parser.add_argument("-w", "--max_workers", type=int, default=4, help="Number of parallel threads (default: 4)")
    ...
    output_path = args.output or os.path.join("results", model_name, "multi_agent_debate", "hle_result.json")
    ...
    print(f"MAD: {correct}/{len(results)} correct -> {output_path}")
